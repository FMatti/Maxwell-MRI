...
omega = 5#.02674011#pow(3.1, 0.5)#np.pi * pow(1/height**2 + 1/(2*width)**2 , 0.5) / (np.pi*2) #width / (np.pi*2) # frequency of input wave

domain = mshr.Rectangle(fen.Point(0.0, 0.0), fen.Point(width, height))# \
       #+ mshr.Rectangle(fen.Point(width/4, height), fen.Point(width/2, height*101/100)) # Additional "cubby" in waveguide
...
class Inlet(fen.SubDomain):
    def inside(self, x, on_boundary):
        return on_boundary and fen.near(x[0], 0) and x[1]>0.0 and x[1]<height
...
helpers.export_field_as_png_plot(A_z, 'A_z_waveguide_2D')
helpers.export_field_as_png_plot(fen.curl(A_z), 'B_waveguide_2D')