...
Let $\mathbf{n}$ and $\mathbf{t}$ denote the normal and tangential
vectors to $\partial \Omega$ at a point $\mathbf{x} \in \partial \Omega$.
...
\begin{equation}
    (({\mu^{-1} \nabla \times \mathbf{A}}) \times \mathbf{v}) \perp \mathbf{n}
\end{equation}

which is satisfied for all $\mathbf{v} \parallel \mathbf{n}$ on $\partial \Omega$.

[Todo: Properly derive/verify this condition using Levi-Civita tensor]
