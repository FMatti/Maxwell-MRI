...
A wide class of problems in physics and engineering concerns itself with studying
the dependence of a model on one of its parameters $\omega$. 
...
My implementations can be found on \cite{git}. Vectors are printed in \textbf{bold}
...
and norms with $||\cdot||$. The Hermitian transpose is written as $^{H}$. For simplicity, results are delivered without units
...
Boundary conditions on the electric field $\mathbf{E}$ along a part of the 
boundary $\Gamma \subset \partial \Omega$ may be most easily enforced
...
representations of $\mathbf{u}, \mathbf{v} \in \mathbb{C}^3$,
...
\acrfull{SVD} of the so-called Gramian matrix $\mathbf{\underline{G}}$, and using the last
...
\citep{shortMRI} tackles both questions simultaneously by sequentially adding new
support points to the build.
...
(\ref{equ:surrogate}) with the nodal polynomial yields
\begin{equation}
    \mathbf{\tilde{u}}(\omega)
...
\end{equation}
In this way, an approximation of the particular relative error norm  required in
Algorithm \ref{alg:gMRI} can be approximated with only Euclidean norms:
\begin{equation}
...
\end{equation}
Notice how on the left-hand side, the vectors of dimension $N$ (usually $N \approx 10000$)
...
looks like, I plot the \acrshort{FEM}-solution at the first and sixth resonant
...
\begin{figure}[ht]
    ...
    \caption{The solution $\mathbf{u}_z$ obtained with the \acrshort{FEM} at the
    sixth resonant frequency $\omega = 4.675$ of the cavity.}
...
studied easy to determine due to the homogeneous boundary conditions:
\begin{equation}
...
are informed about the nature of the source term $\mathbf{f}$, this \enquote{effect}
is usually not accounted for. Even specifying as the initial iteration vector for
the \texttt{eigsh} eigenvalue solver the source term $\mathbf{f}$ to avoid finding
...
\begin{figure}[ht]
    ...
    frequencies $\omega_{n, m}$ appear (or much rather do not appear) whenever the corresponding
    eigensolution $\mathbf{u}_z(\omega_{n, m})$ is orthogonal to the source term $f$ (middle).
    \acrshort{gMRI} (still with a tolerance of $\tau = 10^{-2}$ and 1000 uniformly
    spaced test points in $\omega = [6, 7]$) successfully avoids identifying suppressed resonant frequencies
    while both the analytical and \texttt{eigsh} approach fail to do so.}
...
I now restrict my view to approximating solutions $u_z|_{\Gamma}$ along a trace $\Gamma$. Setting
\begin{equation}
...
matrix representation of the discretized integral form along the trace.
...
\end{equation}
with $\mathbf{I}$ being the matrix representation corresponding to the form
\begin{equation}
