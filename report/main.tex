...
% TODO Report
% - Everywhere in the plot labels u -> \mathbf{u}
% - Figure for u_ring, u_hat, ...
...
study of the dependence of a model on one of its parameters $\omega$. 
...
\acrfull{MRI} offers a way to locally approximate the
quantity $u:\mathbb{C}\ni\omega\mapsto u(\omega)\in H_{\mathrm{curl}}$
that exhibit a meromorphic dependence on one of its 
...
\begin{equation}
    ...
    - \omega^2 \int_{\Omega} \epsilon \mathbf{v}^H \cdot \mathbf{u} = \int_{\Omega} \mathbf{v}^H \cdot \mathbf{j} \label{equ:maxwell-weak-initial}
\end{equation}
...
\begin{fancybox}{Weak formulation of the time-harmonic potential equation}
    \begin{equation}
        \int_{\Omega} (\nabla \times \mathbf{v})^H \cdot ({\mu^{-1} \nabla \times \mathbf{u}})
        - \omega^2 \int_{\Omega} \epsilon \mathbf{v}^H \cdot \mathbf{u}
        = \int_{\Omega} \mathbf{v}^H \cdot \mathbf{j}
        + \int_{\partial \Omega} \mathbf{v}^H \cdot \underbrace{(({\mu^{-1} \nabla \times \mathbf{u}}) \times \mathbf{n})}_{= \mathbf{g}}
...
\begin{equation}
    \int_{\Omega} (\nabla v_z)^H (\mu^{-1} \nabla u_z)
    - \omega^2 \int_{\Omega} \epsilon v_z* u_z
    = \int_{\partial \Omega} v_z* g_z \label{equ:maxwell-weak-resonant-cavity}
\end{equation}
...
\begin{equation}
    a_{\omega}(\mathbf{u}, \mathbf{v}) = \underbrace{\int_{\Omega} (\nabla \times \mathbf{v})^H \cdot (\mu^{-1} \nabla \times \mathbf{u}) }_{K(\mathbf{u}, \mathbf{v})}
    - \omega^2 \underbrace{\int_{\Omega} \epsilon \mathbf{v}^H \cdot \mathbf{u}}_{M(\mathbf{u}, \mathbf{v})} \label{equ:bilinear-form}
\end{equation}
composed with the stiffness form $K(\mathbf{u}, \mathbf{v})$ and the mass form $M(\mathbf{u}, \mathbf{v})$, and
the linear form 
\begin{equation}
    L(\mathbf{u}) = \int_{\Omega} \mathbf{v}^H \cdot \mathbf{j} + \int_{\partial \Omega} \mathbf{v}^H \cdot \mathbf{g} \label{equ:linear-form}
\end{equation}
...
\begin{equation}
    \langle \mathbf{\bar{u}}, \mathbf{\bar{v}} \rangle_M = \mathbf{\bar{u}}^H \mathbf{\underline{M}} \mathbf{\bar{v}} \approx \int_{\Omega} \mathbf{u}^H \cdot \mathbf{v} \label{equ:matrix-inner-product}
\end{equation}
...
\begin{equation}
    ||u||_{M(\Gamma)}^2 = \mathbf{u}|_{\Gamma}^H \mathbf{\underline{M}}(\Gamma) \mathbf{u}|_{\Gamma}
        \approx \int_{\Gamma} \mathbf{u}^H \cdot \mathbf{u}
\end{equation}
